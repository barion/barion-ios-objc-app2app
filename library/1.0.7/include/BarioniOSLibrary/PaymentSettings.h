...
-(id)initWithPOSKey:(NSString*)posKey paymentType:(PaymentType)paymentType guestCheckOut:(BOOL)guestCheckOut fundingSource:(FundingSource)fundingSource locale:(NSString*)locale currency:(NSString*)currency reservationPeriod:(NSDate*)reservationPeriod paymentWindow:(NSDate*)paymentWindow initiateRecurrence:(BOOL)initiateRecurrence recurrenceId:(NSString*)recurrenceId paymentRequestId:(NSString*)paymentRequestId redirectUrl:(NSString*)redirectUrl callbackUrl:(NSString*)callbackUrl orderNumber:(NSString*)orderNumber shippingAddress:(NSString*)shippingAddress debugMode:(BOOL)debugMode redirectUrliOS9:(NSString*)redirectUrliOS9;
